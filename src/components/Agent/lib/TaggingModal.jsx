...
import { useCallCompleted } from "../../Connect/hooks";
import {
  AGENT_NAME,
  genLogger,
  LAMBDA_PREFIX,
...
const getThemes = () =>
  fetch(
    `https://${LAMBDA_PREFIX}.execute-api.ap-southeast-1.amazonaws.com/getThemes`
  ).then((res) => res.json());
...
const sendCallTags = ({ intent, contactId, dropdownValues }) => {
  ...
  const url = new URL(
    `https://${LAMBDA_PREFIX}.execute-api.ap-southeast-1.amazonaws.com/updateIntentTable`
  );
  const params = {
...
const updateDBwithNewCallID = (id) => {
  log("update db", id);
  fetch(
    `https://${LAMBDA_PREFIX}.execute-api.ap-southeast-1.amazonaws.com/newConnectID?agentName=${AGENT_NAME}&connectID=${id}`,
    { method: "post" }
  ).then((res) => res.text());
};
